...
type BusinessImpactMetricProps = {
  customer360BusinesImpactGraphData: any;
  synthesisData: any;
...
const BusinessImpactMetric: React.FC<BusinessImpactMetricProps> = ({
  ...
  const isL3DataAvailable: boolean =
    (Array.isArray(customer360BusinesImpactGraphData?.data?.data) &&
      customer360BusinesImpactGraphData?.data?.data?.some(
        (item: any) => item?.l3_data?.length > 0
      )) ||
    false;
  const { onExpandChartSetting, setOnExpandChartSetting } =
    useOnExpandChartSetting();
  return customer360BusinesImpactGraphData?.data?.data?.length > 0 ? (
    <div className="mx-auto">
      <div className="flex justify-between items-center py-[10px]">
        <p className="h-8 text-[16px] font-medium text-[#141C24] flex justify-center items-center">
          {displayName + ' Metrics' || 'Business Impact Metrics'}
        </p>
      </div>
      {synthesisData?.map((ele: any) => {
        if (
          ele?.type?.toLowerCase() == 'pillar' &&
          ele?.subtype?.toLowerCase() == 'business'
        ) {
          return <Synthesis synthesisData={ele?.synthesis} />;
        }
      })}
      {isL3DataAvailable && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-[22px] gap-y-[22px] border-b border-[#E4E7EC] pb-[30px] min-h-[320px]">
          {customer360BusinesImpactGraphData?.data?.data?.length > 0
            ? customer360BusinesImpactGraphData?.data?.data
...
export default BusinessImpactMetric;
