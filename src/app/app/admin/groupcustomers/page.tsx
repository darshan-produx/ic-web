...
import AddEditGroupCustomerForm from './addeditgroupcustomer';
import { useQuery } from '@tanstack/react-query';
import TableContainer from '../../../common/components/TableContainer';
import { FileEdit, MoreHorizontal, Plus, Search } from 'lucide-react';
import { Dropdown } from '../../../common/Dropdown';
import { getGroupCustomers } from '../../../api/customers/customers';

export default function GroupCustomers() {
  const [openAddEditGroupCustomerForm, setOpenAddEditGroupCustomerForm] =
    useState(false);
  const [editData, setEditData] = useState<any>(null);
  const [searchPh, setSearchPh] = useState('');

  const { data: groupCustomers, isLoading } = useQuery({
    queryKey: ['groupCustomers'],
    queryFn: getGroupCustomers,
  ...
  return (
    <div className="w-full">
      ...
      {openAddEditGroupCustomerForm ? (
        <AddEditGroupCustomerForm
          customer={editData}
          setOpenAddEditGroupCustomerForm={setOpenAddEditGroupCustomerForm}
          openAddEditGroupCustomerForm={openAddEditGroupCustomerForm}
        />
      ) : (
        ''
      )}
      <div className="grid grid-cols-1 gap-x-5 xl:grid-cols-12 mx-4">
        <div className="xl:col-span-12">
          <div className="card" id="usersTable">
              ...
              <div className="flex items-center">
                ...
                <div className="shrink-0">
                  ...
                  >
                    ...
                    <span className="align-middle">Add group customer</span>
                  </button>
                </div>
              </div>
            </div>
            ...
            <div className="card-body">
              {groupCustomers?.data?.data &&
              groupCustomers?.data?.data?.length > 0 ? (
                <TableContainer
                  isPagination={true}
                  columns={columnsUsers || []}
                  data={groupCustomers?.data?.data ?? []}
                  customPageSize={5}
                  // isGlobalFilter={true}
                  searchTerm={searchPh}
