...
import { getUniqueAdoptionBusinessKpiMetrics } from '../../../api/config/adoptoin_business_kpi';
import {
  useAddAdoptionBusinessKPI,
  useEditAdoptionBusinessKPI,
...
export default function AddEditKPIForm({
  ...
  const { data: uniqueMetrics } = useQuery({
    ...
    queryFn: getUniqueAdoptionBusinessKpiMetrics,
    refetchOnWindowFocus: false,
  });
  ...
  const schema = yup
    .object({
      ...
      metric_type: yup
        .string()
        .oneOf(['adoption', 'business', 'Adoption', 'Business'])
  ...
  // ---------- mutations & submit ----------
  const addMutation = useAddAdoptionBusinessKPI();
  const editMutation = useEditAdoptionBusinessKPI();
  ...
  const onSubmit = async (formData: FormValues) => {
    try {
      ...
      if (editData) {
        ...
        queryClient.invalidateQueries({ queryKey: ['adoption-business-kpis'] });
      } else {
