...
import {
  downloadTemplateAdoptionBusinessKpi,
  getAdoptionBusinessKpi,
  templateAdoptionBusinessKpi,
} from '../../../api/config/adoptoin_business_kpi';
...
import {
  useDeleteAdoptionBusinessKpi,
  useUploadAdoptionBusinessKpi,
...
const AdoptionBusinessKpi = () => {
  const [open, setOpen] = useState(false);
  const [adoptionBusinessKpiData, setAdoptionBusinessKpiData] = useState<any>();
  ...
  const { data: existingAdoptionBusinessKpis, isLoading } = useQuery({
    queryKey: ['adoption-business-kpis'],
    queryFn: getAdoptionBusinessKpi,
  ...
  const columns = useMemo(
    () => [
      ...
      {
        header: 'Histrory to consider for trend',
        enableColumnFilter: false,
        cell: (cell: any) => (
          <div
            className="max-w-64"
            title={cell?.row?.original?.history_to_consider_for_trend}
          >
            <div className="truncate">
              {cell?.row?.original?.history_to_consider_for_trend ?? '-'}
            </div>
          </div>
        ),
      },
      ...
      {
        header: 'Momentum threshold',
        enableColumnFilter: false,
        cell: (cell: any) => (
          <div
            className="max-w-64"
            title={cell?.row?.original?.momentum_threshold}
          >
            <div className="truncate">
              {cell?.row?.original?.momentum_threshold ?? '-'}
            </div>
          </div>
        ),
      },
            ...
            >
              ...
              <li
                ...
                onClick={() => {
                  ...
                  setAdoptionBusinessKpiData(cell?.row?.original);
                }}
              >
  ...
  const uploadAdoptionBusinessKpi = useUploadAdoptionBusinessKpi();
  const hadleOnsubmit = async (formData: any) => {
    try {
      const res = await uploadAdoptionBusinessKpi.mutateAsync(formData);
  ...
  const { data: allTemplateData } = useQuery({
    ...
    queryFn: downloadTemplateAdoptionBusinessKpi,
  });
  const { data: templateData } = useQuery({
    queryKey: ['template-adoption-business-kpi'],
    queryFn: templateAdoptionBusinessKpi,
  });
  const handleDownload = () => {
    if (downloadModalOpen) {
      ...
      saveAs(blob, 'adoption_and_business_metric_upload_template.xlsx');
  ...
  const deleteAdoptionBusinessKpi = useDeleteAdoptionBusinessKpi();
  const handleDelete = () => {
    deleteAdoptionBusinessKpi.mutate(adoptionBusinessKpiData?._id);
    handleCloseModal();

    if (deleteAdoptionBusinessKpi) {
      toast.success('Adoption business KPI deleted successfully');
    }
  };
        ...
        <div className="grid grid-cols-1 border-b-[1px] border-[#80c2fe]   pt-4   ">
          <div className="">
            <h6 className="p-2 font-semi-bold text-black text-[32px] text-left">
              Adoption and business impact metrics
            </h6>
          </div>
          <div className=""></div>
        </div>
      ...
      <div className="grid grid-cols-1 gap-x-5 xl:grid-cols-12 mx-4">
        <div className="xl:col-span-12">
          <div className="card">
                ...
                <h6 className="text-15 grow">
                  <div className="block">
                    <h6 className="font-semibold text-[20px] text-black">
                      {`Adoption and business impact metrics  (${existingAdoptionBusinessKpis?.data?.data?.length})`}
                    </h6>
                    <p className="font-normal my-1 text-[14px] tracking-normal leading-5 text-[#cccccc] ">
                      Manage existing adoption and business impact metrics
                    </p>
                  </div>
                </h6>
            ...
            <div className="card-body">
              {existingAdoptionBusinessKpis?.data?.data &&
                existingAdoptionBusinessKpis?.data?.data?.length > 0 ? (
                <TableContainer
                  isPagination={true}
                  columns={columns || []}
                  data={existingAdoptionBusinessKpis?.data?.data ?? []}
                  // customPageSize={10}
                  // isGlobalFilter={true}
                  searchTerm={searchPh}
...
export default AdoptionBusinessKpi;
