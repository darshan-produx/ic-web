...
import { useDeleteNpsMetric } from '../../../services/mutations/configMutations';
import AddNpsMetricForm from './AddNpsMetricForm';
import EditNpsMetricForm from './EditNpsMetricForm';
...
const NPSMetric = () => {
  ...
  const [npsMetricData, setNpsMetricData] = useState<any>();
    ...
    () => [
      {
        ...
        cell: (cell: any) => (
          <div className="max-w-64" title={cell?.row?.original?.nps_target}>
            <div className="truncate">
              {cell?.row?.original?.nps_target ?? '-'}
            </div>
          </div>
        ),
      },
      {
        header: 'NPS threshold',
        enableColumnFilter: false,
        cell: (cell: any) => (
          <div className="max-w-64" title={cell?.row?.original?.nps_threshold}>
            <div className="truncate">
              {cell?.row?.original?.nps_threshold ?? '-'}
            </div>
          </div>
        ),
      },
            ...
            >
              ...
              <li
                ...
                onClick={() => {
                  ...
                  setNpsMetricData(cell?.row?.original);
                }}
              >
  ...
  const deleteNpsMetric = useDeleteNpsMetric();
  const handleCloseModal = () => {
    setDeleteModalOpen(false);
  };

  const handleDelete = useCallback(() => {
    deleteNpsMetric.mutate(npsMetricData?._id, {
      onSuccess: () => {
        toast.success('NPS metric deleted successfully');
        handleCloseModal();
      },
    });
  }, [npsMetricData, deleteNpsMetric]);
  ...
  return (
    <div className="w-full">
      ...
      <div className="grid grid-cols-1 gap-x-5 xl:grid-cols-12 mx-4">
        <div className="xl:col-span-12">
          <div className="card">
                ...
                <h6 className="text-15 grow">
                  <div className="block">
                    <h6 className="font-semibold text-[20px] text-black">
                      {`NPS metrics  (${existingNpsMetrics?.data?.data?.length})`}
                    </h6>
                    <p className="font-normal my-1 text-[14px] tracking-normal leading-5 text-[#cccccc] ">
                      Manage existing NPS metrics
                    </p>
                  </div>
                </h6>
                <div className="shrink-0">
                  <div>
                    {existingNpsMetrics?.data?.data?.length > 0 ? (
                      <button
                        type="button"
                        className="cursor-not-allowed text-white btn bg-slate-500 border-slate-500 hover:text-white hover:bg-slate-600 hover:border-slate-600 focus:text-white focus:bg-slate-600 focus:border-slate-600 focus:ring focus:ring-slate-100 active:text-white active:bg-slate-600 active:border-slate-600 active:ring active:ring-slate-100 dark:ring-slate-400/10"
                      >
                        <Plus className="inline-block size-4" />{' '}
                        <span className="align-middle">Add NPS metric</span>
                      </button>
                    ) : (
                      <button
            ...
            <div className="card-body">
              {existingNpsMetrics?.data?.data &&
              existingNpsMetrics?.data?.data?.length > 0 ? (
                <TableContainer
                  isPagination={true}
                  columns={columns || []}
                  data={existingNpsMetrics?.data?.data ?? []}
                  customPageSize={5}
                  // isGlobalFilter={true}
                  searchTerm={searchPh}
      ...
      {addModal && <AddNpsMetricForm setAddModal={setAddModal} />}
      {editModal && (
        <EditNpsMetricForm setEditModal={setEditModal} editData={editData} />
      )}
      {deleteModalOpen && (
        <DeleteModal
