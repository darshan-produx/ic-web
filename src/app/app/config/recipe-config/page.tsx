...
import {
  useDeleteRecipeConfig,
  useDeleteUsecaseConfig,
  useUploadRecipeConfig,
...
import {
  downloadTemplateRecipeConfig,
  getRecipeConfig,
  templateRecipeConfig,
} from '../../../api/config/recipe_config';

const UsecaseConfig = () => {
  const [open, setOpen] = useState(false);
  const [recipeConfigData, setRecipeConfigData] = useState<any>();
  ...
  const { data: existingRecipeConfigs, isLoading } = useQuery({
            ...
            >
              <li
                ...
                onClick={() => {
                  ...
                  setRecipeConfigData(cell?.row?.original);
                }}
              >
  ...
  const { mutateAsync } = useUploadRecipeConfig();
  ...
  const { data: allTemplateData } = useQuery({
    ...
    queryFn: downloadTemplateRecipeConfig,
  });
  const { data: templateData } = useQuery({
    queryKey: ['template-recipe-config'],
    queryFn: templateRecipeConfig,
  });
  ...
  const handleDownload = async () => {
    if (downloadModalOpen) {
      ...
      saveAs(blob, 'recipe_config.yaml');
  ...
  const deleteRecipeConfig = useDeleteRecipeConfig();
  const handleDelete = () => {
    deleteRecipeConfig.mutate(recipeConfigData?._id);
    handleCloseModal();

    if (deleteRecipeConfig) {
      toast.success('Recipe configuraion deleted successfully');
    }
  };
      ...
      <div className="grid grid-cols-1 gap-x-5 xl:grid-cols-12 mx-4">
        <div className="xl:col-span-12">
          <div className="card">
                ...
                <h6 className="text-15 grow">
                  <div className="block">
                    <h6 className="font-semibold text-[20px] text-black">
                      {`Recipes  (${
                        existingRecipeConfigs?.data?.data?.length ?? 0
                      })`}
                    </h6>
                    <p className="font-normal my-1 text-[14px] tracking-normal leading-5 text-[#cccccc] ">
                      Manage existing recipe configurations
                    </p>
                  </div>
                </h6>
            ...
            <div className="card-body">
              {existingRecipeConfigs?.data?.data &&
              existingRecipeConfigs?.data?.data?.length > 0 ? (
                <TableContainer
                  isPagination={true}
                  columns={columns || []}
                  data={existingRecipeConfigs?.data?.data ?? []}
                  customPageSize={5}
                  // isGlobalFilter={true}
                  searchTerm={searchPh}
